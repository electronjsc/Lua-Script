...
function main()
	...
	sampAddChatMessage("{A52A2A}PLAYER INFO: {32CD32}Äîáàâëåíà âîçìîæíîñòü ñìîòðåòü ìîäåëè Ò/C, {32CD32} Àêòèâàöèÿ *Z* - {A52A2A}NEW")
	...
	while true do wait(0)
		...
		if update_state then
			downloadUrlToFile(script_url, script_path, function(id, status)
				if status == dlstatus.STATUS_ENDDOWNLOADDATA then
					sampAddChatMessage("{A52A2A}PLAYER INFO: {32CD32}Îáíîâëåíèå óñïåøíî ñêà÷àëîñü, îáíîâëÿåì! {40E0D0}Ïîñëåäíÿÿ âåðñèÿ ñêðèïòà: v" .. updateIni.info.vers_text, -1)
					thisScript():reload()
				end
			end)
			break
...
--[[
 function new()
	sampShowDialog(1999, "Íîâûå îáíîâëåíèÿ", "1.Äîáàâëåíà êîìàíäà /new(Ïðîñìîòð îáíîâëåíèé).\n2. Äîáàâëåíà âîçìîæíîñòü ñìîòðåòü íàçâàíèÿ àâòîìîáèëåé(Àêòèâàöèÿ Z).")
		wait(100)
		...
		local result, button, list, input = sampHasDialogRespond(1999)
			if result then
				if button == 1 then
					if list == 0 then
						sampAddChatMessage("Äàííàÿ êîìàíäà áûëà äîáàâëåíà, äëÿ ïðîñìîòðà íîâûõ îáíîâëåíèé, âìåñòå ñ îïèñàííèåì òåõ.")
					end
					if list == 1 then
						sampAddChatMessage("Ïðè çàæèìàíèè Z íà êëàâèàòóðå, íàä àâòîìîáèëåì ïîÿâèòüñÿ áåëàÿ íàäïèñü, è ìîäåëüþ òðàíñïîðòà.")
					end
				end
			end
		end
...
function imgui.OnDrawFrame()
	...
	if main_window_state.v then
		...
		imgui.Text(u8(string.format('Òåêóùàÿ äàòà: %s', os.date("%x"))))
